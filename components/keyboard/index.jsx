...
export default function Keyboard() {
  ...
  const latex = useRef(null);
  const [value, setValue] = useState("");
  const [test, setTest] = useState(0);

  useEffect(() => {
    const mathFieldElement = mf.current;

    console.log(mathFieldElement);
    ...
    if (mathFieldElement) {
      ...
      const focusHandler = () => {
        ...
        const mlContainer = document.querySelector(".ML__keyboard");

        mlContainer.classList.add("flex");

        mlContainer.classList.add("flex-col");

        const mlDiv = document.createElement("div");

        mlDiv.classList.add("flex");
        mlDiv.classList.add("!mt-10");
        mlDiv.classList.add("!bg-white");
        mlDiv.classList.add("z-50");
        mlDiv.classList.add("w-[700px]");

        mlDiv.innerHTML = ReactDOMServer.renderToString(
          <div>
            <h3>Title</h3>
            <p> some message...</p>
          </div>
      ...
      mathFieldElement.addEventListener("focus", focusHandler);

      mf.current.focus();

      return () => {
        mathFieldElement.removeEventListener("input", (evt) => {
          setValue(evt.target.value);
        });

        mathFieldElement.removeEventListener("focus", focusHandler);
      };
    }
  }, [test]);
  ...
  return (
    <div className="flex flex-col gap-4 max-w-[500px] items-center justify-center">
      <math-field ref={mf}>{value}</math-field>
      <p onClick={() => setTest(test + 1)}>Value: {value}</p>
    </div>
  );
}
